...
for Node Classification for mag240m. Please read README_MAG.md
"""
...
def load_dataset(dataset_name: str, root: t.Union[str, Path], subset_frac: float = 0.1) -> dict:
    ...
    Params:
        ...
        subset_frac: float - Fraction of the dataset to load (e.g., 0.001 for 0.1%). (In case you want to work with partial dataset)
    ...
    subset_size = int(len(train_idx) * subset_frac) #this is ideally set to exactly 1
    if subset_size < 1:
        subset_size = 1
...
def match_distribution(
    ...
    for class_label, scaled_count in enumerate(class_distribution_scaled):
        log(f"doing for class {class_label}")
        merged_class_nodes = [
            node
...
# Code block to profile
    ...
    parser.add_argument("--mag240m_root", required=True)
    parser.add_argument(
        "--save",
        default=False,
        action="store_true",
    ...
    if args.dataset == "mag240m":
        dataset = load_dataset(args.dataset, root=args.mag240m_root, subset_frac=args.subset_frac)
    else:
    ...
    nclasses = data.y.max().item() + 1  #note classes should be numbered sequentially
