...
def load_syn_dataset(dataset_name, target_frac, synthetic_root, scaler):
    ...
    adj_file = path / "adj.pt"
    feat_file = path / "feat.pt"
...
def main(args):
    ...
    writer = SummaryWriter(f'logs/GCSR_{args["ds"]}_{args["model"]}_{args["tf"]}')
    from timing import Timer
    with Timer(f"Training GCSR for {args['ne']} epochs") as timer:
