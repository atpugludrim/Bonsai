...
def load_syn_dataset(dataset_name, target_frac, synthetic_root, scaler):
    ...
    eigenval_file = path / f"eigenvals_syn_{exp}.pt"
    eigenvec_file = path / f"eigenvecs_syn_{exp}.pt"
    feat_file = path / f"feat_{exp}.pt"
    label_file = path / f"label_{exp}.pt"
    eigenval = torch.load(eigenval_file, map_location="cpu")
    eigenvec = torch.load(eigenvec_file, map_location="cpu")
    feat = torch.load(feat_file, map_location="cpu")
    label = torch.load(label_file, map_location="cpu")
    n_syn = feat.shape[0]
    adj = torch.eye(n_syn) - eigenvec@torch.diag(eigenval)@eigenvec.T
...
def train_backend_pyg_cuda_clear(model, nepochs, data, data_syn, splits, writer):
    opt = optim.Adam(model.parameters())
    loss_fn = nn.CrossEntropyLoss()
    loop = tqdm(range(nepochs), ascii=True, ncols=120, desc="Training")
    best_acc_val = 0
...
def train_model(model_type, model, nepochs, data, data_syn, splits, writer, dataset):
    ...
    elif model_type == "GINE" and dataset == "ogbn-arxiv":
        train_backend_pyg_cuda_clear(model, nepochs, data, data_syn, splits, writer)
    else:
...
def main(args):
    ...
    writer = SummaryWriter(f'logs/GDEM_{args["ds"]}_{args["model"]}_{args["tf"]}')
    from timing import Timer
    with Timer(f"Training GDEM for {args['ne']} epochs") as timer:
        train_model(model_type, model, args["ne"], data, data_syn, splits, writer, args["ds"])
